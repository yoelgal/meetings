...
# And reachable from the CLI, which looks somewhere else entirely.
#
# `Bundle.module` is generated by SwiftPM, and the accessor it generates looks beside the running
# binary and then at the absolute .build path from the machine that compiled it. The app reaches its
# resources through BundleResources, so Contents/Resources is enough for it. The CLI at
# Contents/Helpers does not: it looks in Contents/Helpers, finds nothing, and falls back to the build
# path — which EXISTS on the machine that built it and is `/Users/runner/work/...` on everybody else's.
#
# So every published release shipped a CLI that fatally errored on any command needing bundled
# resources, and no local test could see it, because locally the fallback resolves. Measured against
# the published v0.4.0-rc1: `meetings skill install` exited 133 with
# "Fatal error: could not load resource bundle", so the agent skill was never installed and the
# install reported success anyway.
#
# A symlink rather than a second copy: the bundle is the same bytes either way, codesign seals a
# symlink inside a bundle without complaint, and `ditto` preserves it through the release archive —
# which is exactly why package-release.sh uses ditto rather than zip.
for b in "${bundles[@]}"; do
    ln -sfn "../Resources/$(basename "$b")" "$CONTENTS/Helpers/$(basename "$b")"
done
...
# Nested code first, then the bundle. The line that stood here said "there is no nested code here, so
# one top-level sign seals everything", and that was simply false: Contents/Helpers/meetings is a
# second Mach-O, and a top-level sign seals it as a RESOURCE without signing it as code. So it shipped
# carrying the linker's ad-hoc signature inside a certificate-signed bundle, and macOS killed its
# first execution with `Trace/BPT trap: 5`.
#
# What that cost, measured against the published v0.4.0-rc1: `meetings skill install` — the first thing
# to run the helper after an install — died, so the agent skill was never installed, the trap text
# leaked into the output, and the install reported success anyway. The second execution onwards worked,
# which is why nothing caught it: the check asserted `meetings status` AFTER the install had already
# spent the failing first exec and discarded it.
#
# Inside-out by hand rather than --deep, which is deprecated: sign the helper, then seal the bundle
# over it. --options runtime is deliberately off until there is a real Developer ID — a local signature
# gains nothing from hardening and the runtime blocks debugging you may want.
codesign --force --sign "$SIGN_IDENTITY" --timestamp=none "$CONTENTS/Helpers/meetings"
